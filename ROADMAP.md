...
> Koden is a terminal-first AI workspace for multi-agent development. See
> `WORKSPACE.md` for the architecture of the workspace layer.
>
> **Shipped**
> - Desktop clipboard UX in the terminal (smart Ctrl+C / Ctrl+X / Ctrl+V, Shift+Enter).
> - Flexible layout: top tab strip or VS Code-style vertical tab rail (resizable).
> - Non-terminal workspace tabs: Markdown notes/scratchpad and kanban boards (persisted).
> - Agent Dock sidebar panel (status, role, model, task, tokens, last activity).
> - Agent Topology graph view, Message Flow timeline inspector, and a Director command dashboard.
> - Orchestration store (spawn / assign / route / review / approve), persisted with the workspace.
>
> **Fork roadmap (next)**
> - Live token/cost metering from the AI SDK + Claude Code agents into the dock/topology; enforce per-agent context and cost limits.
> - Autonomous Director routing through the AI agent loop (not just user-driven).
> - Bridge `managedAgentsStore` (terminal coding-agents) into the orchestration store automatically.
> - Per-space orchestration scoping; drag-and-drop on boards and the topology graph.

Koden direction, what's shipped, what's coming, and what's deliberately out of scope.

This file is updated as direction evolves. For day-to-day work, see [GitHub Issues](https://github.com/MrOlof/koden/issues) and the Projects board.

## What Koden is

Koden is a fast, lightweight, AI-native terminal (ADE - agentic development environment). It pairs a native PTY backend with a modern UI: multi-tab terminals, an integrated code editor, a file explorer, source control, and a first-class AI agent system that works with your own API keys or fully local models. Under 10 MB on disk. No telemetry. Keys stored in the OS keychain.
...
- [ ] AI agent meta-orchestration (Koden agent spawning and managing external coding agents like Claude Code / OpenCode)
...
Categories that will not be built into Koden. Individual feature requests in these categories will be closed.
...
- **Notebook and document workspaces.** Anything that turns Koden into a document host rather than a terminal.
...
- **Telemetry, analytics, accounts.** Koden stays BYOK and offline-respectful.
...
- **Third-party subscription session bridges.** Forwarding cloud subscription auth (provider-managed login sessions) through Koden is not technically feasible for third-party clients.

## Decision authority

Direction and scope decisions are made by [@MrOlof](https://github.com/MrOlof).

If a PR is closed and you disagree, open a GitHub Discussion.
