...
function parseTerminal(raw: unknown, path: string): TerminalPalette | string {
  ...
  if (raw.selectionForeground !== undefined) {
    if (!isStr(raw.selectionForeground))
      return `${path}.selectionForeground must be a string`;
    out.selectionForeground = raw.selectionForeground;
  }
  if (raw.selectionInactive !== undefined) {
    if (!isStr(raw.selectionInactive))
      return `${path}.selectionInactive must be a string`;
    out.selectionInactive = raw.selectionInactive;
  }
