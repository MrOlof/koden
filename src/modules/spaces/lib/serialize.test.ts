...
describe("hydrateTabs", () => {
  ...
  it("round-trips notes, board and orchestration tabs", () => {
    const tabs: Tab[] = [
      { id: 1, kind: "notes", spaceId: "s1", title: "Ideas", docId: "note-x" },
      { id: 2, kind: "board", spaceId: "s1", title: "Sprint", boardId: "board-y" },
    ...
    const notes = restored.find((t) => t.kind === "notes");
    const board = restored.find((t) => t.kind === "board");
    expect(notes?.kind === "notes" && notes.docId).toBe("note-x");
