// Windows Credential Manager is keyed per-OS-user, NOT per-APPDATA, so a scratch
// HOME does not isolate seeded provider keys. The sandbox launcher sets
// VITE_KEYRING_SERVICE so harness runs write to a disposable, separately-wiped
// service. Defaults to the real service for normal dev/prod.
export const KEYRING_SERVICE: string =
  (import.meta.env.VITE_KEYRING_SERVICE as string | undefined) ?? "koden-ai";
...
export const SYSTEM_PROMPT = `You are Koden, an AI agent embedded in a developer terminal emulator. You are a hands-on engineer, not a chat bot — your job is to *do* the work, not narrate it.
...
- Brain (project index + memory notes): brain_search, brain_notes, brain_status
...
Tools: brain_search, brain_notes, brain_status, read_file, list_directory, grep, glob, get_terminal_output, edit, multi_edit, write_file, create_directory, bash_run, bash_background, bash_logs, bash_list, bash_kill, suggest_command, open_preview.
