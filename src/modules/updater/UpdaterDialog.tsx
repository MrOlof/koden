...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      // TODO(koden): publish a Koden AUR package and update this slug (external
      // registry name — left as koden-bin until the AUR package exists).
      return "yay -S koden-bin";
    case "debian":
      return `sudo apt install ./Koden_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Koden-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Koden v${manual.version} is available`
                  : `Koden v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Koden to finish installing."
              : downloading
                ? progress !== null
