...
export function CommandPalette({
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      document.getElementById("koden-command-palette-input")?.focus();
    }, 0);
          ...
          <CommandList className="max-h-none overflow-visible pr-3">
            {inThemes ? (
              ...
              rankedCommands.length === 0 ? (
                ...
                // While searching, render ONE flat list in score order so the
                // best match sits at the very top regardless of its group — e.g.
                // "Add note pane" ranks by relevance, not behind every "Tabs"
                // item. Grouping (below) only kicks in when browsing with no term.
                <CommandGroup heading="Results">
                  {rankedCommands.map((item) => (
                    <ActionItem
                      key={item.id}
                      item={item}
