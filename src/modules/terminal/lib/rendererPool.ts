...
// normal few-terminal usage allocates nothing extra. Past the browser WebGL
// context limit (~16) the per-slot context-loss fallback below degrades that
// slot to DOM rendering — heavier, but still functional and crash-free.
export const POOL_MAX_SIZE = 64;
...
// Dev/test harness only: the WebGL/canvas buffer is unreadable from the DOM, so
// recover terminal text via the already-loaded SerializeAddon. `serializeLeaf`
// keeps ANSI (exact replay); `readLeafBuffer` returns plain text for substring
// assertions. Both return null if the leaf has no live renderer slot.
export function serializeLeaf(leafId: number): string | null {
  const slot = slots.find((s) => s.currentLeafId === leafId);
  if (!slot) return null;
  return slot.serializeAddon.serialize();
}

export function readLeafBuffer(leafId: number): string | null {
  const slot = slots.find((s) => s.currentLeafId === leafId);
  if (!slot) return null;
...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-koden-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  term.loadAddon(
    // Plain left-click copies the URL; Ctrl/Cmd+click opens it in the browser.
    // Matches the smart-link provider's copy-by-default / open-on-modifier model.
    new WebLinksAddon((e, uri) => {
      if (e.ctrlKey || e.metaKey) {
        void openUrl(uri).catch(console.error);
      } else {
        void clipboardWriteText(uri)
  ...
  host.setAttribute("data-koden-slot", String(slots.length));
  ...
  term.attachCustomKeyEventHandler((event) => {
    ...
    if (isTerminalShiftEnter(event)) {
      event.preventDefault();
      if (event.type === "keydown") bridge.writeToPty("\x1b\r");
      return false;
    }
    const clip = terminalClipboardAction(event, {
      isMac: IS_MAC,
        ...
        if (clip === "paste") {
          void clipboardReadText().then((text) => {
            if (text) {
              ...
              // own clipboard-image paste, matching what Alt+V already does.
              bridge.writeToPty("\x1bv");
            }
          });
        } else {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[koden] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[koden] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[koden-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[koden-webgl] dispose failed:", e);
  }
  try {
    const r = (
