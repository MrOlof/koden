...
Koden loads `KODEN.md` from the workspace root as agent memory (similar to AGENTS.md / CLAUDE.md). This file is also the project's living architecture doc — read it before making changes.

## Project

**Koden** — open-source AI-native terminal emulator. Tauri 2 + Rust (`portable-pty`) backend, React 19 + TypeScript + xterm.js (webgl) client, BYOK AI via Vercel AI SDK v6.

- Bundle id: `app.mrolof.koden`
...
- `secrets::secrets_*`: OS keychain via the `keyring` crate. Service constant `koden-ai`. Linux uses a file-based fallback gated behind `#[cfg(target_os = "linux")]`.
- `open_settings_window`: separate webview window for Settings (optional `tab` arg deep-links a section).
- `cli::cli_reply`: the webview's answer to a `koden` CLI request (see the koden CLI section below).
...
- **workspace/** — workspace environment switching (Local + WSL distros + SSH hosts from `~/.ssh/config`). SSH Spaces run the terminal on the host through the system OpenSSH client (`pty/shell_ssh.rs` pushes the rc bundle to `~/.koden/shell` once per content hash); files, git and search refuse ssh envs via `workspace::require_local_fs` until the SFTP/exec layer lands. The env is a property of the Space (`SpaceMeta.env`): `useWorkspaceEnvStore` mirrors the active Space (`app/hooks/useWorkspaceSwitcher.ts`, set synchronously at boot before any PTY spawns), and picking another env from the launcher or the status bar goes to the most recently used Space of that env or creates one (`spaces/lib/envSwitch.ts`) instead of mutating the current Space; nothing disposes the other Spaces' sessions. Remote agent status (M2.8): tmux eats OSC 777, so ssh tabs get their working/attention dots from the host's `~/.koden/pane-events.jsonl` (written by the shared Claude hooks) instead — `ssh_pane_events_start/stop` (ssh.rs) stream a `tail -F` per host over a Channel, `spaces/lib/paneEvents.ts` is the pure core (line validation, the status decision mirroring AgentBusBridge's mid-turn rule, pane→key join via `#{pane_id}` now in `ssh_tmux_windows`), and `spaces/lib/usePaneEvents.ts` owns watcher lifecycle (one per host with terminal tabs, reconnect on drop, TTL'd pane map because tmux reuses `%N`) and escalates `useTabStatusStore` directly.
- **worktrees/** — "New worktree Space" (name to `feat/<slug>`, any base branch, checkout at `<repo>/.koden/worktrees/<slug>`, `worktreeSymlinkPaths` junctioned in) and the Remove flow; a worktree is a Space with `SpaceMeta.worktree`. Rust side in `git/worktree.rs`.
- **launcher/**: the start page (`LauncherPane`): brand block, resume cards (`extraSections`), a START column (open folder, connect to remote as an inline form fed by `ssh_list_hosts` + free text, new terminal local / per WSL distro, new editor, new note), a RECENT column (Spaces by `updatedAt`, `~/…/last/two` paths), a shortcut sheet resolved live from the shortcuts registry, and the version in the footer next to the setup guide. Lives in a `launcher` tab kind that is never persisted; opened on boot (`showLauncherOnStart`), whenever the active Space has no tabs, via `launcher.show` (Mod+N) and the header `+` menu. `lib/launcherItems.ts` is the pure page model (`buildStartPage`, `LauncherSectionModel`; `extraSections` plugs sections in), `lib/useLauncherKeys.ts` walks the rows in document order (resume, START, RECENT; the form and footer are Tab-only).
- **theme/** — custom theme engine (no `next-themes`). `ThemeProvider` + `applyTheme` write CSS variables; built-in presets in `themes/` (koden-default, nord, tide, catppuccin, tokyo-night, caffeine, claude, gruvbox, sage, rose-pine), user themes via `customThemes.ts` + `validateTheme.ts`, optional background image via `bgImageStore.ts` + `SurfaceLayer`.
- **updater/** — auto-updater UI built on `tauri-plugin-updater`.
- **agents/** — agent notifications + management for both the built-in Koden agent and terminal coding-agents (Claude Code; Codex later). Shared store (`store/agentStore.ts`: terminal `sessions` + `localAgent` + `notifications`) and a shared router (`lib/route.ts`: suppress when focused-and-visible, OS-notify when unfocused, in-app Sonner toast when focused-but-hidden) feed the header `NotificationBell` (management surface, Koden agent listed first). Toasts use Sonner (`components/ui/sonner.tsx`) themed via the central engine; `lib/agentIcon.tsx` renders the per-agent brand mark (Koden logo, Claude/Codex hugeicon). Terminal detection is Rust-side (`pty/agent_detect.rs`) on the PTY reader's byte filter, armed on `OSC 133;C;<cmd>`, emitting `koden:agent-signal` transitions (`started`/`working`/`attention`/`finished`/`exited`) driven only by OSC sequences (never raw output, so a repainting TUI never flaps) — zero cost when no agent runs. Terminal signals arrive via Claude Code hooks (`UserPromptSubmit`/`Notification`/`Stop`) returning an `OSC 777` marker through the `terminalSequence` field (hooks lost `/dev/tty` access in v2.1.139); `agent_enable_claude_hooks` installs them (atomic write, never clobbers invalid JSON, prunes empty groups), gated on `KODEN_TERMINAL`, and the marker self-arms the detector so it works in bash/Windows/tmux without shell preexec. The Koden agent path is `ai/components/LocalAgentNotificationsBridge.tsx`, mapping `chatStore.agentMeta` (`awaiting-approval`→attention, busy→idle→finished, `error`) into the same router.
- **cli/** — webview half of the `koden` CLI. `CliBridge.tsx` (mounted once in `App.tsx`) listens for `koden:cli-request`, gates on the Settings > CLI prefs (`cliEnabled`, `cliTerminalRead`, `cliTerminalInput`, `cliPanelControl`, `cliNotify`), runs the pure `lib/dispatch.ts` against a `CliContext` built from the AI live bridge (`chatStore.live`), and answers with `invoke("cli_reply")`. Shares the Librarian's resolver and send shaping (`ai/tools/terminalTargets.ts`, `ai/tools/layoutShared.ts`, SDK-free by design) and reads buffers through `terminal.readLeafTail`. Notifications go through `agents/lib/route.ts`.
- **ai/** — see below.

### koden CLI (`src-tauri/src/modules/cli/`, `src/modules/cli/`)

Every PTY Koden spawns gets `KODEN_EXE`, `KODEN_CLI_ENDPOINT` and `KODEN_CLI_TOKEN`, and each injected shell script defines `koden` (guarded on `KODEN_EXE`, so a copy on a remote host is inert). `koden --help` is the agent-facing reference. The client is the app executable itself: `main.rs` short-circuits `koden cli ...` to `cli::client::run` before any Tauri setup. Never add a second `[[bin]]` for it (the bundler once shipped the wrong executable).

- Transport (`server.rs`): Unix socket `koden-<pid>.sock` (0600) in `$XDG_RUNTIME_DIR` / `dirs::runtime_dir()` / cache dir; Windows named pipe `\\.\pipe\koden-<pid>` (first-instance flag, local clients only). One request per connection, one JSON line each way: `{token,id,cmd,args,session}` in, `{id,ok,result}` or `{id,ok:false,error}` out. Token is 32 hex from OS randomness, checked before args are parsed (`protocol.rs`). Caps: 1 MiB payload each way, 32 in-flight, 64 connections, 30 s reply timeout (`bridge.rs`).
- Bridge: the connection parks on a `Pending` slot, Rust emits `koden:cli-request` (token stripped) to the main window, the webview dispatches and calls `cli_reply(id, ok, result, error)`. `ping` is completed Rust-side with version, pid and endpoint.
- Frontend gate + dispatch live in `src/modules/cli/lib/` (pure, tested with a fake context). Without `--panel` a command targets the calling pane (`KODEN_SESSION` = pty id, `leafIdForPty`); `terminal list` marks it `current`. Privacy and cold panes refuse reads and input; reads are secret-redacted; shell submits pass `checkShellCommand`.
- Windows release exe is GUI-subsystem: `win.rs` attaches the parent console for stdio (`ensure_console`), and `profile.ps1` pipes the call through `Write-Output` so PowerShell waits for it. `win.rs` declares its handful of kernel32 externs by hand to keep Cargo.toml untouched.
- Not in v1: agent orchestration (`agent create --worktree`). The protocol is command-name based (`group.verb`), so new commands are a gate entry + a dispatch case + a Rust `Spec`.
...
- **Key storage**: OS keychain via `keyring` (Rust). Frontend reads/writes through `secrets_*` commands. Service `KEYRING_SERVICE = "koden-ai"`. Never persist keys to disk, settings store, or `localStorage`.
...
- Auto-updater configured with a public minisign key; release artifacts at `https://github.com/MrOlof/koden/releases/latest/download/latest.json`.
...
- **Windows PowerShell process lifecycle**: `killer.kill()` from `portable-pty` only kills the immediate child. Descendants (e.g. `npm run dev` started inside pwsh) survive unless something else takes them down. The Job Object in `pty/job.rs` handles this for the Koden-process-death case; an explicit `pty_close` from JS also kills only the immediate child + relies on the Job to take the rest. Don't disable the Job without a replacement.
