...
- **launcher/** — the "What do you want to do?" page (`LauncherPane`): continue a recent Space, open a terminal (local / WSL), open a folder as a new Space, connect to a remote host (`ssh_list_hosts` + free text), run setup. Lives in a `launcher` tab kind that is never persisted; opened on boot (`showLauncherOnStart`), whenever the active Space has no tabs, via `launcher.show` (Mod+N) and the header `+` menu. `lib/launcherItems.ts` is the pure section/item model (`LauncherSectionModel`, `extraSections` prop for plug-in sections).
