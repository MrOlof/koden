...
- **workspace/** — workspace environment switching (Local + WSL distros + SSH hosts from `~/.ssh/config`). SSH Spaces run the terminal on the host through the system OpenSSH client (`pty/shell_ssh.rs` pushes the rc bundle to `~/.koden/shell` once per content hash); files, git and search refuse ssh envs via `workspace::require_local_fs` until the SFTP/exec layer lands.
