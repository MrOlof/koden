...
let
  ...
  srcMap = {
    x86_64-linux = fetchurl {
      url = "https://github.com/MrOlof/koden/releases/download/v${version}/Koden_${version}_amd64.deb";
      hash = sources.hashes.x86_64-linux;
    };
    x86_64-darwin = fetchurl {
      url = "https://github.com/MrOlof/koden/releases/download/v${version}/Koden_x64.app.tar.gz";
      hash = sources.hashes.x86_64-darwin;
    };
    aarch64-darwin = fetchurl {
      url = "https://github.com/MrOlof/koden/releases/download/v${version}/Koden_aarch64.app.tar.gz";
...
assert lib.assertMsg (builtins.hasAttr sys srcMap)
  "koden: unsupported platform ${sys}";
...
stdenv.mkDerivation {
  ...
  installPhase = if stdenv.hostPlatform.isLinux then ''
    ...
    install -Dm755 usr/bin/koden $out/bin/koden

    wrapProgram $out/bin/koden \
      "''${gappsWrapperArgs[@]}" \
  ...
  meta = with lib; {
    ...
    # ponytail: no Koden website domain decided yet (D3) — repoint when one
    # exists. Using the fork repo URL as a valid placeholder.
    homepage = "https://github.com/MrOlof/koden";
