...
const KODEN_MARKER: &[u8] = b"notify;Koden;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(event) = pt.strip_prefix(KODEN_MARKER) {
...
mod tests {
    ...
    fn koden_marker_drives_status() {
        ...
        assert!(run(&mut d, &osc("777;notify;Koden;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;Koden;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn koden_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
